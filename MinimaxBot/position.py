...
class Position:
    ...
    # return number of microboards won by me if my_turn is true and number of microboards
    # won by opponent if my_turn is false
    def num_boards_won(self, myid, my_turn):
        won = 0
        for x in range(0, 2):
            for y in range(0, 2):
                if getMicroboardWinner(x, y) == myid and my_turn:
                    won += 1
                if getMicroboardWinner(x, y) != myid and not my_turn:
                    won += 1
        return won
